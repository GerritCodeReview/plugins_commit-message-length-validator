...
public class CommitMessageLengthValidation implements CommitValidationListener {
  ...
  public List<CommitValidationMessage> onCommitReceived(CommitReceivedEvent receiveEvent)
    ...
    int longLineCnt = 0;
    int nonEmptyCnt = 0;
    int subjectLineCnt = 0;            // subject can span multiple lines
    ...
    // find message body by taking total lines - (all footers lines + all subject lines)
    boolean bodyExists =
        (nonEmptyCnt - (commit.getFooterLines().size() + subjectLineCnt)) > 0 ? true:false;
    if (!bodyExists) {
